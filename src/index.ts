...
import { fetchModels, NIM_API_BASE } from './providers/nvidia-nim';
import { getGenerator, listGenerators } from './generators/registry';
import { classifyModel } from './generators/continue';
...
async function resolveApiKey(flagValue: string | undefined): Promise<string> {
...
program
  ...
  .option(
    '-k, --api-key <key>',
    'NVIDIA NIM API key (or set NVIDIA_API_KEY env var)'
  )
  .option('-o, --output <path>', 'Output path for the config file')
  .option('--overwrite', 'Overwrite an existing config file', false)
  .option(
    '--all-models',
    'Skip model selection and include all available chat/code models',
    false
  )
  .action(
    async (
      ...
      if (!generator) {
        ...
        logger.info(
          `Available targets: ${listGenerators()
            .map((g) => g.id)
            .join(', ')}`
        );
        process.exit(1);
      ...
      try {
        ...
        logger.error(`Failed to fetch models: ${(err as Error).message}`);
        process.exit(1);
      }
      ...
      if (!options.allModels) {
        const { chosen: selectedIds } = await inquirer.prompt<{
        ...
        }>([
          {
            ...
            name: 'chosen',
            message:
              'Select models to include (space to toggle, enter to confirm):',
        ...
        selectedModels = usableModels.filter((m) =>
      ...
      // ── Security notice ─────────────────────────────────────────────────
      logger.warn(
        'Your NVIDIA NIM API key will be stored in plain text in the config file.'
      );
      ...
      try {
        ...
        logger.info(
          `Restart your ${generator.name} extension / IDE to apply the new configuration.`
        );
      } catch (err) {
